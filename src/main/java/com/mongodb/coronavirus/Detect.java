...
public class Detect extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        if(DetectOnOff.filterOn && !isBotMessage(e)){
            ...
            for(int i = 0;i < message.length;i++){
                ...
                for(int b = 0; b < LIST_OF_MYTHS.length;b++){
                    if (message[i].equalsIgnoreCase(LIST_OF_MYTHS[b])){
                        ...
                        myth = true;
                        if(DetectOnOff.filterOn){ //Prints a message IF enabled by ~detectfakenews
    ...
    public EmbedBuilder getResponse(GuildMessageReceivedEvent event){
        ...
        embed.setFooter("author: " + event.getAuthor().getName());
        return embed;
    }
    public boolean isBotMessage(GuildMessageReceivedEvent event){
